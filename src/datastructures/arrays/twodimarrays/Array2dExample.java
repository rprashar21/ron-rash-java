...
public class Array2dExample {
    public static void main(String[] args) {
        ...
        int sumofDaignol = 0;
        // sum of diagnol elemnents
        for(int row =0;row<a.length;row++){
            for(int col =0;col<a[row].length;col++){

                // sum of daignol
                if(row==col)
                    sumofDaignol=sumofDaignol+a[row][col];
            }
        }
        System.out.println("sum of daignol is " + sumofDaignol);

        int sumofrevDaignol = 0;
        // sum of diagnol elemnents
        for(int row =a.length-1;row>=0;row--){
            for(int col =a[row].length;col>=0;col--){

                // sum of daignol
                if(row==col)
                    sumofrevDaignol=sumofrevDaignol+a[row][col];
            }
        }
        System.out.println("sum of rev daignol is " + sumofrevDaignol);
    }
